...
namespace DFramework.MyStorage.Web
{
    ...
    public class ExceptionManager : IExceptionManager
    {
        ...
        protected virtual string UnknownMessage { get; set; } = "-1";// ErrorCode.UnknownError.ToString();
            ...
            do
            {
                ...
                catch (Exception ex)
                {
                    if (!(ex is OptimisticConcurrencyException) || !needRetry)
                    {
                        ...
                        //else
                        {
                            apiResult = new ApiResult(-1, getExceptionMessage(ex));
                            //Logger?.Error(ex);
                        }
                        needRetry = false;
                    }
            ...
            do
            {
                ...
                catch (Exception ex)
                {
                    if (!(ex is OptimisticConcurrencyException) || !needRetry)
                    {
                        ...
                        //else
                        {
                            apiResult = new ApiResult<T>(-1, getExceptionMessage(ex));
                            //Logger?.Error(ex);
                        }
                        needRetry = false;
                    }
